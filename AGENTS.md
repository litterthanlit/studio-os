...
- **M1 — It understood me (design-brain master plan Phase 1):** **CODE COMPLETE** on [draft PR #23](https://github.com/litterthanlit/studio-os/pull/23), 2026-09-24. Tasks 1.1–1.10 each landed as one commit with its gate.
  - **Engine and memory.** One server-side pipeline for the editor, agents and the benchmark (`lib/engine`), with checkpoints and resume in `generationRuns`. Design Memory tables (taste layers, token sets, briefs, reference analyses, preferences).
  - **Understanding.** Measured palette, grid and type scale. Perception with evidence. Briefs with per-reference roles, conflicts and ≤ 3 questions. Layered taste compile with provenance (HARD only from measured or explicit values). Scoped preferences learned from design actions.
  - **Models.** `modelFor(route)` routing (the judge is a different family from the generator), a cacheable prompt prefix, and streamed base generation (Live Build).
  - **UI (approved by Nick).** Intent sliders + Restyle, the Reference X-ray (X), role chips (1–7), region lasso, Intent Card and Taste Memory panel, and Live Build sections with flat skeletons.
  - **Open:** the live M1 demo and the live cached-token check need a deployment and keys. See `SESSION.md`.
- **M0 — Trustworthy (design-brain master plan Phase 0):** **CODE COMPLETE** on [draft PR #23](https://github.com/litterthanlit/studio-os/pull/23), 2026-09-23. Tasks 0.1–0.10 each landed as one commit with its gate.
  - **Rendering and intent.** Real fonts in screenshots and exports. Word-boundary plus model intent classification. Taste signature and image identity. Persisted corrections (HARD palette, `userOverrides.knobs`, `generationBaseline`). In-app prompt frame with status colors.
  - **Agents.** Server-side design state for agents. Async agent runs with `get_run`. Rebase on revision conflict.
  - **Storage and telemetry.** References in file storage, plus a canvas-limits proof. Hygiene: `npm run verify` is green. Model-call telemetry.
  - **Open:** the live Convex run for 0.6 (offline analysis: a 12-level tree exceeds the 16-level limit) and the M0 live demo, both of which need a deployment and keys. See `SESSION.md`.
...
- **Phase 5 (Agent platform v1):** **SHIPPED**. V6 generation core extracted to `lib/canvas/generate-design-core.ts`; `request-design` calls it directly (no Bearer-only proxy). Project-scoped agent auth via `authorizeAgentProjectAccess` + Convex service-secret admin path. New APIs: `/api/agent/canvas`, `/api/agent/projects`, `/api/agent/screen-design`, `/api/agent/generate-screen`, `/api/agent/review-implementation`. MCP server expanded with `list_projects`, `get_canvas`, `get_screen_design`, `generate_screen`, `review_implementation` (legacy 3 tools retained). Proof: `npm run proof:agent-platform`. Manual P5 QA pending (Cursor MCP + live generate_screen).
- **Phase 4 (Convex canvas source of truth):** **SHIPPED** (track 1 tightened 2026-09-09). Editor and agents persist through `persistCanvasState` on one `canvasDocuments` row; `prepareCanvasDocumentSave` / `applyCanvasDocumentWrite` are the shared serializers. localStorage is cache/offline draft only — a different (including newer) remote revision always wins. Proof: `npm run proof:convex-canvas-sync`. Manual P4 QA still useful (cross-tab + agent write while editor is open).
...
- **Phase 5a (Export):** COMPLETE (superseded by Track 10 for full handoff). Copy HTML with inline styles, ExportTab in inspector.
...
- **Track 7 (Advanced Multi-Edit / Shared Properties):** COMPLETE. Property comparison engine with early-exit optimization, batch update actions with single history entry, mixed-value UI ("—" for numbers, checkerboard for colors, unselected segments), smart section filtering. Inspector refactored for multi-select (single code path). Performance: 0.002ms for 50 nodes. 6 commits, proof gate 25/25 passed.
- **Track 8 (Direct Layout Manipulation):** CODE COMPLETE — **coordinate fix shipped** (container-local rects + `zoom` divide, tree-ordered flex children). Manual QA pending per `docs/superpowers/plans/2026-04-07-track8-implementation-plan.md`.
- **Track 9 (Component System Maturity):** **SHIPPED** in `studio-os`. Override literacy (helpers, instance root summary, reset all, layers affordances), master edit **returnTo** selection, detach/history, **named presets** (Option B: `presets` + `presetId`, Hero Dark builtin), `SET_INSTANCE_PRESET`, promote preserves presets, **resolve memo** (`computeComponentsResolveEpoch` + `masterEditDirty` full-deps). Plan/spec: `docs/superpowers/plans/2026-04-07-track9-component-maturity-plan.md`, `docs/superpowers/specs/2026-04-07-track9-component-system-maturity-spec.md`.
- **Track 10 (Export / Handoff / Publish):** CODE COMPLETE in `studio-os`. **Phase A:** `ExportOptions` (scope, fragment/document, desktop vs active artboard), `resolveTree` for export, external-image warning, Copy HTML, Download ZIP (`jszip`). **Phase B:** `POST /api/export/publish`, Supabase `published_exports` + migration, public `GET /published/[id]`, Publish link in Export tab (auth required). **QA pending:** [spec §9 + §11.4](docs/superpowers/specs/2026-04-08-track10-export-handoff-spec.md). Apply DB migration before publish works in prod.
- **Effects Stack (Figma-like shadows/blur):** CODE COMPLETE in `studio-os`. Canonical `DesignNodeStyle.effects` (`dropShadow`, `innerShadow`, `layerBlur`, `backgroundBlur`), legacy `shadow`/`blur` compatibility bridge, inspector `EFFECTS` panel (add/toggle/type/reorder/remove + typed controls), renderer/export CSS mapping (`box-shadow`, `filter`, `backdrop-filter`), prompt schema + validator sanitization updates.
- **Editor maturity docs:** [editor-maturity-tracks.md](docs/superpowers/specs/2026-04-04-editor-maturity-tracks.md), [Track 9 spec/plan](docs/superpowers/specs/2026-04-07-track9-component-system-maturity-spec.md), [Track 10 spec/plan](docs/superpowers/specs/2026-04-08-track10-export-handoff-spec.md).
- **Geist/shadcn primitives + prompts:** [spec + proof gates P1–P5](docs/superpowers/specs/2026-04-09-geist-shadcn-primitives-prompts-spec.md) — Phases 1–3 + **4A** in code (CEO tranche: secondary button, link CTA, icon+label row; prompt-only accent mapping for premium-saas).
- **Dashboard theming + profile sync fix (2026-04-09):** **SHIPPED** in `studio-os` (`8c27d12` on `main`). Home greeting and sidebar profile label now subscribe to shared profile storage/events (`lib/profile-store.ts`) so Settings name changes reflect immediately (same-tab + cross-tab). Projects page rendering aligned to semantic theme tokens for dark mode parity.
- **Canvas-Level Creation Tools (Backlog B):** **SHIPPED** in `studio-os`. Single-level model (industry standard — Figma/Framer/Paper parity). FrameItem + TextItem in CanvasItem union. `activeArtboardId` → `activeItemId` generalized selection. 6 new reducer actions. CanvasFrame/CanvasText renderers via ComposeDocumentViewV6. Canvas-level useFrameDraw/useTextPlace. Cmd+C/X/V cross-context clipboard. Drag-to-reparent with drop-zone highlight. Inspector + layers panel support. 14 commits, 3 new files, ~20 modified. No impact on renderer, generation, taste harness, or export. Spec: `docs/superpowers/specs/2026-04-09-canvas-level-creation-tools-design.md`.
- **Harness UX (5 features):** **SHIPPED** in `studio-os`. The taste loop is closed. F3: Weighted references (star/default/mute, weight-aware extraction). F1: Section-level regeneration (right-click similar/different, prompt panel mode, sibling context). F2: Variant comparison (1+1 derivation, carousel tabs, PICK_VARIANT). F5: Generation-as-reference (html-to-image capture, auto-starred primary). F4: Feedback loop (taste edit detection at generation boundary, Apply All/Skip dialog, userOverrides as hard constraints). 15 commits, 4 new files. Spec: `docs/superpowers/specs/2026-04-09-harness-ux-design.md`.
...
| 2026-04-09 | **Track 10** export + publish in `studio-os`: `ExportOptions`, resolved-tree HTML, fragment/document, ZIP (`jszip`), external-image warnings; Phase B `POST /api/export/publish`, `GET /published/[id]`, `published_exports` migration. Proof gates §9 + §11.4 and Track 9 export reconciliation pending per spec. |
| 2026-04-09 | **Effects Stack** in `studio-os`: shipped Figma-like ordered `effects[]` across editor, renderer, export/publish parity, and generation schema/validator; includes legacy `shadow`/`blur` migration bridge. |
| 2026-04-09 (later) | Dashboard fixes shipped to `studio-os/main` (`8c27d12`): profile-name update from Settings now reflects live on Home and sidebar via shared profile store/events; Projects page rendering repaired for dark theme (tokenized surfaces/borders/text). |
| 2026-04-09 (session 2) | **Canvas-Level Creation Tools shipped** (Backlog B resolved). Single-level architecture matching Figma/Framer/Paper. FrameItem/TextItem types, 6 reducer actions, CanvasFrame/CanvasText renderers, F/T tools, Cmd+C/X/V clipboard, drag-to-reparent, inspector + layers panel. 19 commits. |
| 2026-04-09 (session 2 cont.) | **Harness UX — all 5 features shipped.** Taste loop closed. Weighted references (star/default/mute). Section-level regen (similar/different + prompt panel). Variant comparison (1+1 carousel). Gen-as-reference (html-to-image capture). Feedback loop (taste edit detection + Apply All/Skip dialog + userOverrides). 15 commits. Root directory archived (122 files → docs/archive/). |
...
| 2026-09-24 | **M1 (Phase 1) code complete** on PR #23: engine pipeline, Design Memory, Intent Engine (measure, perceive, brief), layered taste compile, learned preferences, model routes plus a cacheable prefix, Live Build streaming, intent sliders, and X-ray / role chips / Intent Card / Taste Memory. 10 task commits, each with its `proof:*` gate. |
