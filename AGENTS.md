...
- **Editor shell (ongoing):** `EditorTransportBar` unifies tools + zoom + Generate + undo/redo (replaces separate ToolPalette/BottomBarV3). Persisted **Light | Dark | System** editor chrome (`useEditorTheme`); **dark mode** uses a dark infinite canvas workspace (`--canvas-workspace`, `.editor-canvas-surface` dot grid); artboard document surfaces stay light. Inspector: clearer radius (all corners + px) and padding (side + corner-pair labels). See `CLAUDE.md` + `docs/editor-hybrid-mapping.md`.
