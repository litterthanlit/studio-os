...
export async function GET(req: Request) {
  ...
  try {
    let endpoint: string;
    if (query.trim()) {
      endpoint = `${LUMMI_API}/images/search?query=${encodeURIComponent(query.trim())}&per_page=${limit}&page=${Math.floor(offset / limit) + 1}`;
    } else {
      endpoint = `${LUMMI_API}/images/random?count=${limit}`;
    }
