...
// ── Route ───────────────────────────────────────────────────────────────────
export type TasteExtractResult =
  | { ok: true; profile: TasteProfile; cached: boolean }
  | { ok: false; status: number; error: string };

/**
 * Extract a TasteProfile from references (cached per project + input signature).
 * Shared by POST /api/taste/extract and the engine pipeline.
 */
export async function extractTasteProfile(body: TasteExtractBody): Promise<TasteExtractResult> {
  try {
    ...
    if (referenceUrls.length === 0) {
      return { ok: false, status: 400, error: "referenceUrls are required" };
    }
    ...
    if (!apiKey) {
      ...
      return { ok: true, profile: fallback, cached: false };
    }
